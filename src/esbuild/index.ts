...
export async function runEsbuild(
  ...
  try {
    result = await esbuild({
      ...
      splitting,
      logLevel: 'error',
      // target 为 es5 的时候，压缩交给 swc 去做，减少构建时间？理论上可以减少，但是实际效果微乎其微
      minify: options.target === 'es5' ? undefined : options.minify,
      minifyWhitespace: options.target === 'es5' ? undefined : options.minifyWhitespace,
      minifyIdentifiers: options.target === 'es5' ? undefined : options.minifyIdentifiers,
      minifySyntax: options.target === 'es5' ? undefined : options.minifySyntax,
